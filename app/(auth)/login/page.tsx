  ...
  return (
    <AuthLayout slogan={t("auth.slogan")}>
      <Box sx={{ width: "100%", textAlign: "start" }}>
        ...
        {/* DEMO REPO ONLY: one-click sign-in for the three personas. Calls the same
            onSubmit as the form, so the demo path and the typed path are the same
            code — including the celebration tick and the role-aware redirect. */}
        <DemoCredentialsPanel
          busy={loading || isRedirecting}
          onSelect={(persona) =>
            onSubmit({ email: persona.email, password: DEMO_PASSWORD })
          }
        />
