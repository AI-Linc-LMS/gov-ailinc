...
const BUCKETS = [
  { label: "0–25%", min: 0, max: 25, color: "#b32020" },
  { label: "25–50%", min: 25, max: 50, color: "#b7791f" },
  { label: "50–75%", min: 50, max: 75, color: "#1b4f8a" },
  { label: "75–100%", min: 75, max: 100.01, color: "#0e7a3c" },
];
    ...
    <PageShell>
      ...
      {!error && (
        <>
          <KpiRail
            items={[
              { value: `${dash?.avg_progress ?? 0}%`, label: "Avg progress", accent: "#1b4f8a" },
              { value: `${dash?.completion_rate ?? 0}%`, label: "Completed", accent: "#0e7a3c" },
              { value: dash?.active_students ?? 0, label: "Active (7d)", accent: "#0f6b7a" },
              { value: dash?.at_risk_count ?? 0, label: "At risk", accent: "#b32020" },
            ]}
          />
          ...
          <Box data-tour-id="instructor-analytics" sx={{ mt: 3.5, p: { xs: 2, md: 3 }, borderRadius: 3, bgcolor: "var(--card-bg)", border: "1px solid var(--border-default)" }}>
            <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 2 }}>
              <Icon icon="mdi:chart-bar" width={20} style={{ color: "#0f6b7a" }} />
