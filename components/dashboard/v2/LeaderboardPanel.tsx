...
export function LeaderboardPanel({ leaderboard }: { leaderboard: DashboardLeaderboard }) {
  ...
  return (
    <PanelCard>
        ...
        <Stack direction="row" spacing={1.25} alignItems="center">
          ...
          <Box>
            <Typography sx={{ fontWeight: 800, color: "#0f172a", fontSize: "0.95rem", lineHeight: 1.1 }}>
              You&apos;re #{me.rank}
              {delta > 0 && <Box component="span" sx={{ color: "#0b6232", fontWeight: 800, ml: 0.5, fontSize: "0.8rem" }}>▲ +{delta}</Box>}
              {delta < 0 && <Box component="span" sx={{ color: "#8f1919", fontWeight: 800, ml: 0.5, fontSize: "0.8rem" }}>▼ {delta}</Box>}
            </Typography>
        ...
        {leaderboard.rows.map((r) => {
          ...
          return (
            <Stack
              ...
              sx={{ p: 0.85, borderRadius: 2.5, bgcolor: r.is_current_user ? "#eef3fa" : "transparent", border: r.is_current_user ? "1px solid #eef3fa" : "1px solid transparent" }}
            >
              ...
              <Typography sx={{ flex: 1, minWidth: 0, fontWeight: r.is_current_user ? 800 : 600, fontSize: "0.85rem", color: "#0f172a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {r.name}{r.is_current_user && <Box component="span" sx={{ color: "#14406f", fontWeight: 700 }}> (you)</Box>}
              </Typography>
      ...
      {leaderboard.aiTip && (
        <Stack direction="row" spacing={0.6} alignItems="flex-start" sx={{ mt: 1.25, p: 1, borderRadius: 2, background: "linear-gradient(135deg, #eef3fa, #eef5f6)" }}>
          <Icon icon="mdi:star-four-points" width={13} color="#164274" style={{ flexShrink: 0, marginTop: 2 }} />
          <Typography sx={{ fontSize: "0.74rem", color: "#164274", fontWeight: 600, lineHeight: 1.45 }}>{leaderboard.aiTip}</Typography>
        </Stack>
      )}
    </PanelCard>
  );
