...
const CALIB_STATUS_CHIP: Record<string, { label: string; color: string; bg: string }> = {
  done: { label: "DONE", color: "#14532d", bg: "#2f9159" },
  not_started: { label: "NOT STARTED", color: "#1e293b", bg: "#c9903a" },
  not_configured: { label: "SETUP PENDING", color: "#1e293b", bg: "#cbd5e1" },
  generating: { label: "PREPARING", color: "#0e2a4b", bg: "#b6cde8" },
};
  ...
  return (
    ...
    >
      <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
        <Stack direction="row" spacing={1.25} alignItems="center">
          <Box sx={{ width: 38, height: 38, borderRadius: 2.5, display: "grid", placeItems: "center", bgcolor: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)" }}>
            <Icon icon="mdi:shield-half-full" width={20} color="#c07033" />
          </Box>
          <Box>
      ...
      <Stack direction="row" flexWrap="wrap" gap={0.75} sx={{ mt: "auto", pt: 1.75 }}>
        ...
        {card.questionCount > 0 && <Pill dark icon="mdi:help-circle" iconColor="#c94b4b" label={`${card.questionCount} fixed Qs`} />}
        <Pill dark icon="mdi:trophy" iconColor="#c9903a" label={`${card.points} pts`} />
        {card.proctored && <Pill dark icon="mdi:shield-account" iconColor="#b6cde8" label="Self-proctored" />}
        {card.proctored && <Pill dark icon="mdi:fullscreen" iconColor="#c9903a" label="Go full screen" />}
      </Stack>
...
function InterviewerCard({ interview, courseId }: { interview: JourneyBoard["interview"]; courseId: number }) {
  ...
  return (
    <Box
      sx={{
        flex: 1, minWidth: 280, p: 2.5, borderRadius: 4, border: "1px solid #d9e6f4",
        display: "flex", flexDirection: "column",
        backgroundImage: "radial-gradient(120% 120% at 100% 0%, #eef3fa 0%, #ffffff 45%)",
      }}
    >
      <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
        <Stack direction="row" spacing={1.25} alignItems="center">
          <Box sx={{ p: "2px", borderRadius: "50%", background: "linear-gradient(135deg, #14406f 0%, #0b5260 100%)" }}>
            <Box sx={{ width: 36, height: 36, borderRadius: "50%", bgcolor: "#fff", display: "grid", placeItems: "center" }}>
              <Icon icon="mdi:star-four-points" width={18} color="#1b4f8a" />
            </Box>
          </Box>
          <Box>
            <Stack direction="row" spacing={0.75} alignItems="center">
              ...
              <Chip
                ...
                label="LIVE"
                size="small"
                sx={{ height: 20, fontSize: "0.6rem", fontWeight: 800, color: "#fff", background: "linear-gradient(135deg, #14406f, #0b5260)", "& .MuiChip-icon": { color: "#fff", ml: 0.5 } }}
              />
              {status === "done" && <Chip label="DONE" size="small" sx={{ height: 20, fontSize: "0.6rem", fontWeight: 800, color: "#14532d", bgcolor: "#c8e6d5" }} />}
            </Stack>
            <Typography sx={{ fontSize: "0.76rem", color: "#64748b" }}>Practice rounds, on demand</Typography>
          </Box>
        </Stack>
        <ButtonBase disabled={!configured} onClick={launch} sx={{ p: 0.5, borderRadius: "50%", color: "#1b4f8a", "&.Mui-disabled": { color: "#cbd5e1" } }}>
        ...
        {chips.map(({ t, hot }) => (
          <Box
            ...
            sx={{
              ...
              bgcolor: hot ? "#eef3fa" : "#f1f5f9",
              color: hot ? "#164274" : "#334155",
              border: hot ? "1px solid #d9e6f4" : "1px solid #e2e8f0",
            }}
          >
            {t}
          </Box>
      ...
      <Stack direction="row" spacing={1.5} alignItems="center" sx={{ mt: 2 }}>
        <ButtonBase
          ...
          sx={{
            ...
            gap: 0.75, background: configured ? "linear-gradient(135deg, #14406f 0%, #0b5260 100%)" : "#cbd5e1",
            boxShadow: configured ? "var(--shadow-sm)" : "none",
          }}
        >
